...
@objcMembers @objc class CustomizationController: NSObject {
  ...
  /// The shared controller used to customize VideoEditor SDK.
  static let shared = CustomizationController()

  /// Applies the customizations to the SDK.
  ///
  /// In here, the default `VideoEditViewController` is exchanged with a
  /// custom `CustomVideoEditViewController` and the default `MenuCollectionViewCell` is exchanged
  /// with a custom `CustomMenuCollectionViewCell` that allows to display a premium feature indicator.
  func apply() {
    do {
      try VESDK.replaceClass(VideoEditViewController.self, with: CustomVideoEditViewController.self)
      try VESDK.replaceClass(MenuCollectionViewCell.self, with: CustomMenuCollectionViewCell.self)
    } catch let error {
      print("Customizations could not be applied, due to:", error.localizedDescription)
    }
  }

  /// A custom implementation of the `VideoEditViewController`.
